...
globals [first-sick interactions simticks  message ]
  ...
  [
   ...
   [
     ...
     if p > 0 [
      ...
      let them substring line (p + 1) (length line)
      show "entries"
      show me
      show them
      ifelse (me = last-username) 
      [
        ask turtles with [ id = me ] [ interact them 0 ]
      ];case of existing 
      [
        ask turtles with [ id = me ] [ user-message (word "Turtle " id " has duplicate data " ) die ]
        create-turtles 1 [
         set id me
         move-to one-of patches with [ abs pxcor < max-pxcor - 4  and abs pycor < max-pycor - 3 ]
         set size 3
         set color green
         set simx xcor
         set simy ycor
...
to interact [partnerid arisk ]
  ifelse (table:has-key? interaction-table partnerid) 
  [
    let histwithpartner table:get interaction-table partnerid
    let instanceindex (length histwithpartner)
    set histwithpartner lput (list partnerid arisk -1 instanceindex) histwithpartner
    table:put interaction-table  partnerid  histwithpartner
    set interaction-history lput (list partnerid arisk -1 instanceindex) interaction-history
  ]
  [
    let histwithpartner (list partnerid arisk -1 0) 
    table:put interaction-table  partnerid  (list histwithpartner)
    set interaction-history lput (list partnerid arisk -1 0) interaction-history
  ]
end
...
to replay-outbreak
  ...
  ask first-sick [ set color red ]
  if do-layout? [ ask first-sick [ setxy 0  0 ] ]
  set sick-ones (turtle-set sick-ones first-sick)
  set message (word "FIRST SICK = " [id] of first-sick)
  display
  user-message message
  ask first-sick [ 
    foreach interaction-history [ 
      let sturtles turtles with [ id = item 0 ? ] 
      ask sturtles [  update-infected-by first-sick 0  if (do-layout?) [ create-link-with first-sick [ set thickness .2] ]  ] 
      set other-sick-ones (turtle-set other-sick-ones sturtles)  
    ] 
  ]
  ...
  let slist []
  ask other-sick-ones [ if not member? self sick-ones [ set slist lput id slist]  ]
  set message (word count other-sick-ones " infected by " [id] of first-sick ": " slist)
  display
  user-message message
  ...
  [
    ...
    ask sick-ones [ 
      ...
      [
        ...
        ask sturtles [ update-infected-by myself (last inter)  if (do-layout?) [ create-link-with myself [ set thickness .2] ] ] 
    ...
    set slist []
    ask other-sick-ones [ if not member? self sick-ones [ set slist lput id slist]  ]
    set message (word count other-sick-ones " infected in round " rcount ": " slist)
    display
    user-message message
    set sick-ones (turtle-set sick-ones other-sick-ones)  
  ]
...
414
621
show interaction history of selected agent
